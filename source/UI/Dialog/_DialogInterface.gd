...
func load_dialog(dialog_text: String = ''):
	...
	if dialog[current_phase].required_flag != '':
	...
	if (not making_choice and requirements_met and dialog_index < dialog[current_phase].dialogue.size()) or dialog_text:
		...
		if dialog[current_phase].start_flag != '' and Globals.flags[dialog.start_flag] == null:
			print_debug("Starting ", dialog.start_flag)
	...
	if dialog_text == '' and not making_choice:
		dialog_index += 1
