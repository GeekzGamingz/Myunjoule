...
#Ready
func _ready() -> void:
	$HarshWeather/CollisionShape2D.set_deferred("disabled", true)
#-------------------------------------------------------------------------------------------------#
#Applies Gravity
...
func flipMouskie():
	if oldFacing != Facing:
		if motion.x < 0:
			...
			get_node("HarshWeather/CollisionShape2D").position = Vector2(-24, 3)
...
#Attack
func attackEnable():
	$HarshWeather/CollisionShape2D.set_deferred("disabled", false)
func attackDisable():
	$HarshWeather/CollisionShape2D.set_deferred("disabled", true)
